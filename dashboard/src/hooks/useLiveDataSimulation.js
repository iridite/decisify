 ...
 */
export const useLiveDataSimulation = (initialData, isActive = false, speed = 1) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSimulatedData((prevData) => {
        ...
        // 1. Update Polymarket odds (slow drift)
        if (newData.perception?.polymarket) {
          ...
          // Update delta
          const oldOdds = newData.perception.polymarket.history[
          ...
          newData.perception.polymarket.volume_24h = Math.floor(
            newData.perception.polymarket.volume_24h * (0.95 + Math.random() * 0.1)
          );
        }

        // 2. Update X Intelligence sentiment scores
        if (newData.perception?.x_intelligence) {
          newData.perception.x_intelligence = newData.perception.x_intelligence.map(
            (tweet) => ({
              ...tweet,
              sentiment_score: randomWalk(tweet.sentiment_score, 0.2, 0.95, 0.03),
        ...
        // 3. Update Nautilus trading data
        if (newData.perception?.nautilus) {
          ...
          newData.perception.nautilus.daily_pnl += priceChange * 0.1;
        ...
        if (newData.triangulation_matrix) {
          ...
          newData.triangulation_matrix.overall_alignment =
            (newData.triangulation_matrix.polymarket_x_correlation +
             newData.triangulation_matrix.x_nautilus_correlation) / 2;
        }

        // 5. Occasionally add new context events
        if (Math.random() < 0.1 && newData.context_memory?.events) {
          const eventTypes = [
            "POLYMARKET_SPIKE",
            "X_SENTIMENT_SHIFT",
            "VOLATILITY_SPIKE",
    ...
    }, 2000 / speed); // Adjust interval based on speed
